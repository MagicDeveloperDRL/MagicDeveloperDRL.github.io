...
SQL语句可以单行或多行书写，以分号结尾；不区分大小写；添加空格和换行符不影响。
...
- xxx.ibd: xxx代表表名，InnoDB 引擎的每张表都会对应这样一个**表空间文件**，存储该表的**表结构**（frm、sdi）、**数据**和**索引**。
...
- InnoDB: 如果应用对事物的完整性有比较高的要求，在**并发条件**下要求数据的一致性，数据操作除了插入和查询之外，还包含很多的更新、删除操作，则 InnoDB 是比较合适的选择
- MyISAM: 如果应用是以读操作和插入操作为主，只有很少的更新和删除操作，并且对事务的完整性、并发性要求不高，那这个存储引擎是非常合适的。
- Memory: 将所有数据保存在内存中，访问速度快，通常用于临时表及缓存。Memory 的缺陷是对表的大小有限制，太大的表无法缓存在内存中，而且无法保障数据的安全性

电商中的足迹和评论适合使用 MyISAM 引擎，缓存适合使用 Memory 引擎，绝大部分场景都使用InnoDB。
...
假如查询数据时表中没有索引，则需要全表扫描，查询效率非常低，如果具有索引则可以不需要全表扫描。
...
在将某一列设置为主键约束时，就会自动创建一个主键索引

在将某一列设置为唯一约束时，就会自动创建一个唯一索引
...
回表查询就是先走二级索引找到数据的ID值，然后根据ID值再找聚集索引找到对应的数据
